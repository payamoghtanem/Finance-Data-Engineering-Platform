...
- **Current phase:** Phase 1 — Local MVP, **in progress** (EPIC-01 done, EPIC-02 mostly done, EPIC-03 done, EPIC-04 mostly done, EPIC-05 mostly done, EPIC-06 done, EPIC-07 mostly done, EPIC-08 mostly done). `main` branch-protected. Phase 1 backlog is live as GitHub Issues (§8). 133 unit tests, 91% coverage, plus a real `dbt build` (31 seed/model/test steps) and `dagster definitions validate` in CI.
...
12. [ ] Write `docs/architecture/capacity-model.md` — data volumes at 12/36 months. NFR targets currently have no load model behind them.
13. [ ] Write `docs/architecture/threat-model.md` — STRIDE pass over ingestion, API, agent, secrets.
14. [ ] Create `runbooks/` + `runbook-template.md` (required by NFR-MAINT-001).
15. [ ] Write `Dockerfile.dagster` (and fix/verify `infra/docker-compose.yml`'s `dagster` service end-to-end) — needs a Docker daemon to validate, not available in this session.
16. [ ] Build the Grafana operator dashboard + alert-rule provisioning that EPIC-08 deliberately deferred — needs a Docker daemon to verify, not available in this session; see DEBT-10.
