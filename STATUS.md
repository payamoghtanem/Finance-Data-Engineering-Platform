...
- **Current phase:** Phase 1 — Local MVP, **in progress** (EPIC-01 done, EPIC-02 mostly done, EPIC-03 done, EPIC-04 mostly done, EPIC-05 mostly done, EPIC-06 done, EPIC-07 mostly done). `main` branch-protected. Phase 1 backlog is live as GitHub Issues (§8). 104 unit tests, 90% coverage, plus a real `dbt build` (31 seed/model/test steps) and `dagster definitions validate` in CI.
...
14. [ ] Write `Dockerfile.dagster` (and fix/verify `infra/docker-compose.yml`'s `dagster` service end-to-end) — needs a Docker daemon to validate, not available in this session.
