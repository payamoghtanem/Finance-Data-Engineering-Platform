...
- **Current phase:** Phase 1 — Local MVP, **in progress** (EPIC-01 done, EPIC-02 mostly done, EPIC-03 done, EPIC-04 mostly done, EPIC-05 mostly done, EPIC-06 done, EPIC-07 mostly done, EPIC-08 mostly done, EPIC-09 mostly done, EPIC-10 partial). `main` branch-protected. Phase 1 backlog is live as GitHub Issues (§8). 157 unit tests, 91% coverage, plus a real `dbt build` (31 seed/model/test steps), a Gold query-latency check, and `dagster definitions validate` in CI. **Documentation debt cleared**: capacity-model.md, threat-model.md, and runbooks/ template + RUNBOOK-002 all complete (DEBT-03/04 closed).
...
14. [x] ~~Write `docs/architecture/capacity-model.md` — data volumes at 12/36 months. NFR targets currently have no load model behind them.~~ — **Done 2026-09-11**. Projections at 12/24/36 months for storage/compute/network, cost estimates, scaling triggers, optimization strategies. Closes DEBT-03.
15. [x] ~~Write `docs/architecture/threat-model.md` — STRIDE pass over ingestion, API, agent, secrets.~~ — **Done 2026-09-11**. Full STRIDE analysis with 28 threats mapped to likelihood/impact, mitigations tied to NFR-SEC/NFR-AVAIL requirements, risk matrix, compliance mapping. Closes DEBT-04.
16. [x] ~~Create `runbooks/` + `runbook-template.md` (required by NFR-MAINT-001).~~ — **Done 2026-09-11**. Template created per NFR-MAINT-001, plus first operational runbook (RUNBOOK-002: Capacity Scaling) demonstrating the template in action.
...
| DEBT-03 | ~~No capacity/volume model; NFR targets unanchored to load~~ | Low | **Resolved 2026-09-11** — `docs/architecture/capacity-model.md` created with 12/24/36 month projections, cost estimates, scaling triggers. |
| DEBT-04 | ~~No threat model despite "secure by default" being ARD principle 7~~ | Low | **Resolved 2026-09-11** — `docs/architecture/threat-model.md` created with full STRIDE analysis (28 threats), risk matrix, compliance mapping. |
