...
- **Phase 1**: `src/events/bus.py`'s `EventBus` — synchronous, in-process publish/subscribe (EPIC-06). No external broker; a subscriber only ever learns of upstream work through a `publish()` call, never a bare "the script ran" signal (US-06-001). Currently wired as each producer's own module gets built: `connectors/fred` publishes `ingestion.requested`/`raw_data.received`; `bronze/` publishes `bronze_data.written`; `validation/` (EPIC-04) publishes `raw_data.validated`/`raw_data.quarantined` when `ValidationEngine.validate()` is called directly. What's still missing is the wiring between them: nothing yet calls `ValidationEngine.validate()` in response to a real `raw_data.received` event (that needs a source-specific payload parser, per `../technical/technical-design-document.md` §2c), and nothing yet subscribes to `raw_data.validated` to trigger `bronze/`. The remaining producers in the catalog above (Dagster, Silver/Gold) don't exist yet — see `STATUS.md` for what's actually done.
