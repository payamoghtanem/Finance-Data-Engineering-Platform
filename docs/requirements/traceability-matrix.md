...
| NFR-PERF-001..003 | §4 Value proposition: reproducible, fast analysis | `NFR.md` §4 | NFR-PERF-001 (Gold dashboard query < 5s): `scripts/verify_epic10_gold_query_latency.py` (EPIC-10) seeds a synthetic multi-year fixture through the real Bronze/dbt path, runs the query Metabase's dashboard card would issue against `main_gold.fact_economic_kpi`, and asserts median latency under the 5s target — **partial**: proves the query-latency mechanism on a laptop-sized fixture (no capacity model exists yet, DEBT-03), not real production data volumes or Metabase's own rendering overhead on top of it (no live Metabase instance in this session, DEBT-11); NFR-PERF-002/003 unaddressed (no API yet, EPIC-11) | CI's `dbt-transform` job runs `verify_epic10_gold_query_latency.py` on every push |
